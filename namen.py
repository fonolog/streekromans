...
for filen in filenames:
    fnamen = {}
    print(filen)
    with open(filen,'r') as open_file:
        doc = open_file.readlines()
        for line in doc:
            for word in line.split():
                total_words += 1
                if word in voornamen and not word.lower() in STOPWORDS:
                    if word in fnamen:
                        fnamen[word]+=1
                    else: fnamen[word]=1
        for n in fnamen:
            if n in namen: 
                namen[n] =(fnamen[n]+namen[n][0], 1+namen[n][1])
            else: 
                namen[n] = (fnamen[n],1)
...
for s in enumerate(sorted(namen.items(), key=lambda item: item[1][1], reverse=True)):
        pretty_result += (f"{s[0]+1}.\t{s[1][0]}:\t{s[1][1]}\n")

print(pretty_result, file=open('voornamen2.txt','w'))
